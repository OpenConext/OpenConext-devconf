...
class ApiFeatureContext implements Context
{
    ...
    /**
     * If the value contains multiple values, please provide them comma seperated.
     *
     * If you want to test if an empty array was provided (which is a perfectly valid setting) leave the expectation
     * empty
     *
    ...
    public function instituteHasAPropertyWhichEquals($institute, $property, $expectedValue)
    {
        ...
        assertEquals(
            ...
            $actualValue,
            sprintf(
                "Asserting the [%s] property in current scope equals [%s]. Payload: %s",
                $property,
     ...
     */
    public function theApiResponseStatusCodeShouldBe($statusCode)
    {
        $response = $this->getLastResponse();
        ...
        assertEquals(
            ...
            sprintf(
                'Expected status code "%s" does not match observed status code "%s"',
                $statusCode,
                $response->getStatusCode()
            )
        );
    ...
    public function printLastResponseOnError(AfterScenarioScope $scope)
    {
        if ($scope->getTestResult()->getResultCode() == TestResult::FAILED) {
            ...
            if (in_array(
                $this->lastResponse->getHeader('Content-Type'),
